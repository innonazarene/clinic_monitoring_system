...
<body>
    <div class="header">
        ...
        <h2>Clinic Monitoring System - Treatment Summary Report</h2>
    </div>
    <div class="meta">
        <strong>Period:</strong> {{ $dateFrom }} to {{ $dateTo }}
        @if($department)
            | <strong>Department:</strong> {{ $department->full_name }}
        @endif
    ...
    <div class="summary-box">
        <table>
            ...
            <tbody>
            @foreach($illnessCounts as $diagnosis => $count)
                <tr><td>{{ $diagnosis }}</td><td>{{ $count }}</td></tr>
            @endforeach
            </tbody>
        </table>
    </div>

    <h3 class="section-title">Treatment Records ({{ $treatments->count() }} total)</h3>
    <table>
        <thead>
            <tr>
